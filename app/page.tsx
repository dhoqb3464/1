  ...
  <div className="p-4 space-y-2">
    {countries.map((country) => (
      <button
        ...
        className={`w-full flex items-center justify-between p-4 rounded-xl transition-all duration-200 active:scale-[0.98] touch-manipulation border ${
          selectedCode === country.code
            ? 'bg-white/10 border-white/10 shadow-lg'
            : 'bg-transparent border-transparent active:bg-white/15'
        }`}
      >
        <div className="flex items-center gap-3 flex-1 min-w-0">
          <CountryFlag countryCode={country.code} className="w-10 h-7 shrink-0" />
          <span className={`text-[16px] tracking-tight truncate ${
            selectedCode === country.code ? 'text-white font-semibold' : 'text-white/80'
          }`}>
            {country.name}
          </span>
        </div>
        {selectedCode === country.code && (
          <Icon name="check" className="w-5 h-5 text-[#409CFF] shrink-0 ml-2" />
        )}
      </button>
    ))}
  </div>
